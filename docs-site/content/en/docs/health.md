...
**Note:** The `example.yaml` shows a slightly different structure with `recovery_timeout_seconds` and `triggers`. The actual implementation uses `open_duration_ms` and `half_open_probes` as shown above.
