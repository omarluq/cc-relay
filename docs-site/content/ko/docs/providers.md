...
Model Mapping은 Anthropic 모델 이름을 Z.AI 동등 모델로 변환합니다. Claude Code가 `claude-sonnet-4-6`를 요청하면 cc-relay가 자동으로 `GLM-4.7`로 라우팅합니다:
...
| claude-haiku-4-5 | $0.25 입력 / $1.25 출력 | GLM-4.5-Air | ~$0.04 / $0.18 |
...
1. **버전 접미사 포함**: `claude-sonnet-4-5`와 `claude-sonnet-4-6` 둘 다 매핑
