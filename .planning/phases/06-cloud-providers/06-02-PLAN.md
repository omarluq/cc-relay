...
Output: BedrockProvider implementation with SigV4 signing and comprehensive unit tests.
</objective>
...
<task type="auto">
  <name>Task 2: Implement BedrockProvider</name>
...
3. **SupportsTransparentAuth() bool:** Return false (uses AWS creds, not client tokens)

4. **sha256Hex(data []byte) string:** Helper to compute hex-encoded SHA256

Do NOT modify request body to add anthropic_version yet - that will be handled in the proxy handler transformation layer (future enhancement). The provider focuses on authentication.
  </action>
  <verify>go build ./internal/providers/... compiles</verify>
  <done>BedrockProvider struct with Authenticate method using SigV4</done>
</task>
...
3. **TestBedrockProvider_SupportsTransparentAuth:**
   - Verify returns false

4. **TestBedrockProvider_BaseProvider:**
...
```
  ...
  <done>Unit tests cover constructor, authentication, and inherited methods</done>
</task>
