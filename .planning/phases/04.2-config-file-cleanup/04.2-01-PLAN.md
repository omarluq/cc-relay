...
must_haves:
  ...
  artifacts:
    ...
    - path: "llms.txt"
      ...
      contains: "example.yaml"
    - path: ".claude/skills/auto-fix-code.md"
      provides: "Auto-fix skill documentation"
      contains: "yamlfmt -w example.yaml"
    - path: ".claude/skills/fix-format-issues.md"
      provides: "Fix format skill documentation"
      contains: "yamlfmt -w example.yaml"
  key_links:
...
3. Update `.claude/CLAUDE.md` line 350:
   - FROM: `./cc-relay serve --config config/example.yaml`
   - TO: `./cc-relay serve --config example.yaml`

4. Update `llms.txt` line 79:
   - FROM: `└── config/example.yaml            # Example configuration`
   - TO: `└── example.yaml                     # Example configuration`

5. Update `llms.txt` line 145:
   - FROM: `[config/example.yaml](./config/example.yaml)`
   - TO: `[example.yaml](./example.yaml)`

6. Update `.claude/skills/auto-fix-code.md` line 194:
   - FROM: `yamlfmt -w config/example.yaml`
   - TO: `yamlfmt -w example.yaml`

7. Update `.claude/skills/fix-format-issues.md` line 249:
   - FROM: `yamlfmt -w config/example.yaml`
   - TO: `yamlfmt -w example.yaml`
...
# Verify CLAUDE.md reference updated (line 334)
grep -q 'See `example.yaml` for full reference' .claude/CLAUDE.md && echo "PASS: CLAUDE.md reference updated"

# Verify CLAUDE.md example command updated (line 350)
grep -q '\./cc-relay serve --config example.yaml' .claude/CLAUDE.md && echo "PASS: CLAUDE.md example command updated"
...
grep -q 'yamlfmt -w example.yaml' .claude/skills/auto-fix-code.md && echo "PASS: auto-fix-code.md updated"
grep -q 'yamlfmt -w example.yaml' .claude/skills/fix-format-issues.md && echo "PASS: fix-format-issues.md updated"

# Verify no remaining config/example.yaml references in docs
...
- .claude/CLAUDE.md references example.yaml (not config/example.yaml) in both line 334 and line 350
- llms.txt references example.yaml (not config/example.yaml)
- .claude/skills/auto-fix-code.md references example.yaml (not config/example.yaml)
- .claude/skills/fix-format-issues.md references example.yaml (not config/example.yaml)
...
- [ ] .claude/CLAUDE.md references example.yaml correctly (both line 334 and line 350)
- [ ] llms.txt references example.yaml correctly
- [ ] .claude/skills/auto-fix-code.md references example.yaml correctly
- [ ] .claude/skills/fix-format-issues.md references example.yaml correctly
