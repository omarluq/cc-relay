...
Purpose: Property-based testing generates hundreds of random inputs to verify invariants hold. The three packages chosen (keypool, ratelimit, auth) have the most complex state-dependent logic in cc-relay:
- **keypool**: Key selection involves capacity tracking, priority ordering, and fairness - edge cases exist at boundary conditions
- **ratelimit**: Token bucket has time-sensitive state, concurrent access, and burst handling - subtle bugs lurk in timing
- **auth**: Chain evaluation has short-circuit logic and fallback behavior - order-dependent bugs are easy to miss
